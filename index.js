...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    return "Помилка-не передано жодного елементу";
  }
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    const firstElem = arr.indexOf(element);
    const lsatElem = arr.lastIndexOf(element);
    const result = [];
    if (firstElem !== -1) {
      result.push(firstElem);
    }
    if (firstElem !== lsatElem) {
      result.push(lsatElem);
    }
    return result;
  } else {
    const newArr = [];
...
console.log(findElementIndexes([1, 2, 3, 4, 5, 2], 1)); //Виведе [ 1, 5 ]
...
function customEvery(arr, condition) {
  ...
  if (Array.isArray(arr) === true && typeof condition === "function") {
...
/**
 * Функція `customFill` заповнює вказаний діапазон елементів масиву заданою значенням
 *  за допомогою методу `fill`.
...
function customShift(arr) {
  ...
  if (Array.isArray(arr) === true && 0 < arr.length) {
...
/**
 * Функція `customUnshift` додає один або більше елементів на початок масиву за допомогою методу
 *  `unshift`.
...
function customUnshift(arr, ...elements) {
  ...
  // при кожній ітерації лічильник зменшуємо на 1 та продовжуємо поки лічильник більше,
  //або дорівнює нулю
  ...
  if (Array.isArray(arr) === true) {
    ...
    for (let i = elements.length - 1; 0 <= i; ) {
    ...
    return "It`s not a array";
  }
}

console.log("Завдання: 14 ==============================");

console.log(customUnshift([2, 3, 4, 5], 1, 0));
// Виведе { initialLength: 4, newLength: 6, arr: [ 1, 0, 2, 3, 4, 5 ] }
...
function customAt(arr, index) {
  ...
  // перевіремо якого типу елемент ми отримали,
  //якщо рядок виведом в консоль -Елемент є рядком,
  //числом - Елемент є числом, об`єктом -Елемент є обєктом
  if (Array.isArray(arr) === true && index <= arr.length) {
    let testElement = typeof arr[index];
    if (testElement === "number") {
      console.log ( 'Елемент є числом');
		return  arr[index];
    } else if (testElement === "string") {
		console.log ( "Елемент є рядком");
      return arr[index];
    } else if (testElement === "object") {
...
function customIncludes(arr, element) {
  ...
  //та виводимо число в консоль
  //Повертаємо результат
if (Array.isArray(arr)===true&&arr.includes(element)===true){
const result=(value)=>value===element;
